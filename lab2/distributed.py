...
async def create_poll(poll: Poll):
    if poll.id in polls:
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content="Poll already exists")
    polls[poll.id] = poll
    return poll
...
# async def vote(poll_id: int, answer: str):
#     if poll_id not in polls:
#         return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": "Poll not found"})
#     if answer not in polls[poll_id].answers:
#         return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"error": "Invalid answer"})
#     votes.append({"poll_id": poll_id, "answer": answer})
#     return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "Vote accepted"})

@app.post("/polls/{poll_id}/vote/")
async def vote(vote2: Vote):
    if vote2.poll_id not in polls:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": "Poll not found"})
    if vote2.option not in polls[vote2.poll_id].answers:
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"error": "Invalid answer"})
    votes.append(vote2)
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "Vote accepted"})
...
async def update_vote(vote: Vote):
    if vote.poll_id not in polls:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": "Poll not found"})
    if vote.option not in polls[vote.poll_id].answers:
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"error": "Invalid answer"})

    global votes
    votes = [v for v in votes if v.poll_id != vote.poll_id]
    votes.append(vote)
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "Vote updated"})
...
async def get_results(poll_id: int):
    if poll_id not in polls:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": "Poll not found"})
    result = {answer: votes.count({"poll_id": poll_id, "answer": answer}) for answer in polls[poll_id].answers}
    return result
